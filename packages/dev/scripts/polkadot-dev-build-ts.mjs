...
function tweakPackageInfo (buildDir) {
  ['js', 'cjs'].forEach((ext) => {
    ...
    fs.writeFileSync(fileName, contents.replace("type: 'auto'", `type: '${ext === 'cjs' ? 'cjs' : 'esm'}'`));
  });
}
  ...
  if (!fs.existsSync(path.join(process.cwd(), '.skip-build'))) {
    ...
    if (!name.startsWith('@polkadot/x-')) {
      ...
      if (!fs.existsSync(cjsRoot)) {
        fs.mkdirSync(cjsRoot);
      }

      fs.writeFileSync(path.join(process.cwd(), 'src/cjs/dirname.d.ts'), `${genHeader}\ndeclare const __dirname: string | undefined;\n\nexport default __dirname;\n`);
      fs.writeFileSync(path.join(process.cwd(), 'src/cjs/dirname.js'), `${genHeader}\nmodule.exports = typeof __dirname === 'string'\n  ? __dirname.replace('/cjs', '')\n  : undefined;\n`);
      fs.writeFileSync(path.join(process.cwd(), 'src/cjs/package.json'), '{\n  "type":"commonjs"\n}');
    }
